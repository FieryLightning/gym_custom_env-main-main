...
class WarehouseRobotEnv(gym.Env):
    ...
    def __init__(self, grid_rows=4, grid_cols=5, render_mode=None):
        ...
        self.observation_space = spaces.Box(
            low=0,
            high=np.array([self.grid_rows-1, self.grid_cols-1, math.pi*2, self.grid_rows-1, self.grid_cols-1]),
            shape=(5,),
            dtype=np.float32
        )
    ...
    def step(self, action):
        ...
        # Construct the observation state: 
        # [robot_row_pos, robot_col_pos, target_row_pos, target_col_pos]
        # [robot_row_pos, robot_col_pos, robot_ target_row_pos, target_col_pos]
        obs = np.concatenate((self.warehouse_robot.robot_pos, [self.warehouse_robot.robot_facing_angle], self.warehouse_robot.target_pos))
        # Additional info to return. For debugging or whatever.
        info = {}
...
if __name__=="__main__":
    ...
    while(True):
        ...
        if keys[pygame.K_a]:
            man_action = 0
            obs, reward, terminated, _, _ = env.step(man_action)
        if keys[pygame.K_w]:
            man_action = 1
            obs, reward, terminated, _, _ = env.step(man_action)
        if keys[pygame.K_d]:
